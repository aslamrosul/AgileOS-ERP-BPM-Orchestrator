...
export default function NewCustomerPage() {
  ...
  const [formData, setFormData] = useState({
    customer_name: '', customer_type: 'individual', tax_id: '', contact_person: '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      const response = await authenticatedFetch(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/accounting/customers`, {
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        <Link href="/accounting/customers" className="inline-flex items-center gap-2 text-emerald-600 hover:text-emerald-700 mb-4">
          <ArrowLeft className="w-4 h-4" /><span>Back to Customers</span>
        </Link>
        <h1 className="text-3xl font-bold text-gray-900">New Customer</h1>
        <p className="text-gray-600 mt-2">Create a new customer</p>
      </div>
      <form onSubmit={handleSubmit} className="max-w-4xl space-y-6">
        ...
        <div className="flex gap-3">
          <Link href="/accounting/customers" className="flex-1 flex items-center justify-center gap-2 px-6 py-3 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors">
          ...
          <button type="submit" disabled={loading}
            ...
            {loading ? <><div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div>Creating...</> : <><Save className="w-4 h-4" />Create Customer</>}
          </button>
        </div>
      </form>
    </div>

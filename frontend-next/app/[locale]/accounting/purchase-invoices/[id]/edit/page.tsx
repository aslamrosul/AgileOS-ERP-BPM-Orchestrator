...
import { 
  Save, X, Plus, Trash2, Building, Calendar, FileText, 
  DollarSign, Percent, AlertCircle, ArrowLeft, Calculator, Edit
} from 'lucide-react';
...
export default function EditPurchaseInvoicePage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const [invoiceRes, vendorsRes, accountsRes] = await Promise.all([
        authenticatedFetch(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/accounting/purchase-invoices/${params.id}`),
      ...
      // Pre-fill form data
      setVendorId(invoiceData.vendor_id);
      setInvoiceDate(new Date(invoiceData.invoice_date).toISOString().split('T')[0]);
      ...
      if (invoiceData.lines && invoiceData.lines.length > 0) {
        setLines(invoiceData.lines.map((line: any) => ({
      ...
      if (vendorsRes.ok && accountsRes.ok) {
        ...
        setAccounts((accountsData || []).filter((a: Account) => a.account_type === 'expense'));
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    if (vendorId && invoiceDate) {
      ...
      if (vendor && vendor.payment_terms) {
        ...
        date.setDate(date.getDate() + vendor.payment_terms);
        setDueDate(date.toISOString().split('T')[0]);
      }
    }
  }, [vendorId, invoiceDate, vendors]);
  ...
  const calculateTotals = () => {
    ...
    const taxAmount = lines.reduce((sum, line) => {
      const lineTax = (line.quantity * line.unit_price) * (line.tax_rate / 100);
      return sum + lineTax;
    }, 0);
  ...
  const handleSubmit = async (status: 'draft' | 'submitted') => {
    ...
    if (lines.some(line => !line.description || !line.account_id)) {
    ...
    try {
      ...
      toast.success(`Purchase invoice ${status === 'draft' ? 'updated' : 'submitted'} successfully`);
      router.push(`/accounting/purchase-invoices/${params.id}`);
    } catch (error) {
      ...
      <div className="mb-8">
        <div className="flex items-center gap-4 mb-4">
          ...
          <div>
            ...
            <p className="text-gray-600 mt-1">Editing invoice {invoice.invoice_number}</p>
          </div>
        </div>
        ...
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-medium text-yellow-900">Draft Invoice</p>
              <p className="text-sm text-yellow-700 mt-1">
                Only draft invoices can be edited. Once submitted, invoices cannot be modified.
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                {selectedVendor && (
                  <p className="text-xs text-gray-500 mt-1">
                    Payment Terms: {selectedVendor.payment_terms} days
                  </p>
                )}
              </div>
              ...
              <div className="md:col-span-2">
                ...
                <textarea value={description} onChange={(e) => setDescription(e.target.value)}
                  placeholder="Invoice description or notes" rows={2}
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500" />
              </div>
            </div>
          </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div className="md:col-span-2">
                      ...
                      <input type="text" value={line.description}
                        ...
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500" required />
                    </div>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 sticky top-8">
            ...
            {selectedVendor && (
              <div className="mb-6 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                <p className="text-xs font-medium text-blue-900 mb-1">Vendor Balance</p>
              ...
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-yellow-700 mt-1">
                    Changes will be saved to the existing invoice. Submitted invoices require approval before payment.
                  </p>
                </div>
              </div>
            </div>

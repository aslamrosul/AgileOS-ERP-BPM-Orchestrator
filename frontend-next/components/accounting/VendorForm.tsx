...
export default function VendorForm({ vendor, mode, onClose, onSuccess }: VendorFormProps) {
  ...
  const validateForm = (): boolean => {
    ...
    } else if (formData.vendor_name.length < 3) {
      newErrors.vendor_name = 'Vendor name must be at least 3 characters';
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = mode === 'edit' && vendor?.id
        ? `${process.env.NEXT_PUBLIC_API_URL}/api/v1/accounting/vendors/${vendor.id}`
      ...
      toast.success(`Vendor ${mode === 'edit' ? 'updated' : 'created'} successfully`);
      onSuccess();
      onClose();
    } catch (error: any) {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-white">
                {mode === 'create' && 'Create New Vendor'}
                {mode === 'edit' && 'Edit Vendor'}
                {mode === 'view' && 'Vendor Details'}
              </h2>
              <p className="text-emerald-100 text-sm">
                {mode === 'create' && 'Add a new vendor to your system'}
                {mode === 'edit' && 'Update vendor information'}
                {mode === 'view' && 'View vendor details'}
              </p>
            </div>
          </div>
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              <label className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">Vendor is active and can be used in transactions</p>
                </div>
              </label>
            </div>
          </div>
            ...
            >
              {saving ? (
                ...
                <>
                  ...
                  <span>{mode === 'edit' ? 'Update Vendor' : 'Create Vendor'}</span>
                </>
              )}
            </button>
          )}
